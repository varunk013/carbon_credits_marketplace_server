...
const { RequiredVaulesValidator } = require("../../utils/requiredValuesChecker");
...
const handleSignIn = async (request, response) => {
    try {
        ...
        if (isValid) {
            ...
            const accessToken = await JWT.sign({ firstName, lastName, email }, process.env.JWTACCESSKEY, { expiresIn: '1h' });
            const refreshToken = await JWT.sign({ firstName, lastName, email }, process.env.JWTREFRESHKEY, { expiresIn: '10h' });

            response.cookie('accessToken', accessToken, { maxAge: 3600, httpOnly: true });
            response.cookie('refreshToken', refreshToken, { maxAge: 36000, httpOnly: true });
